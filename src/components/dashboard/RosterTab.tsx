  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-5 animate-in fade-in-50 duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-card/60 backdrop-blur-md p-5 rounded-2xl border border-border shadow-lg">
        <div>
          <h2 className="text-xl font-black text-rose-450 flex items-center gap-2 drop-shadow-sm">
        ...
        <Button
          variant="outline"
          onClick={() => restRoster()}
          className="bg-emerald-950/40 text-emerald-400 border-emerald-500/30 font-bold px-4 py-2 hover:bg-emerald-900/60 transition-all font-mono shadow-sm flex items-center gap-2 text-xs"
        >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className={`font-black text-sm drop-shadow-sm transition-colors ${isSelected ? 'text-white' : 'text-foreground group-hover:text-primary'}`}>
                      {p.summonerName}
                    </h4>
                  </div>
                </div>
              ...
              const renderActionButton = (programId: 'LANING' | 'SOLOQ' | 'MACRO' | 'SCRIMS' | 'MINDSET' | 'REST', defaultText: string, customDisabled: boolean) => {
                ...
                if (programId === 'REST') {
                  ...
                  if (isLoading) {
                    ...
                    btnClass = "bg-rose-500/20 text-rose-450 border border-rose-500/40 cursor-default min-w-[124px]";
                    btnContent = (
                      <span className="flex items-center justify-center gap-1.5 font-sans">
                        <AlertCircle className="h-3.5 w-3.5" />
                        지도 불가
              ...
              return (
                <div className="flex flex-col xl:flex-row gap-6 h-full">
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="bg-background/50 rounded-xl border border-border p-4 flex-1 min-h-[200px] relative">
